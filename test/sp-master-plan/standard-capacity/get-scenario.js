...
var Manager = require("../../../src/managers/sp-master-plan/standard-hour-manager");
var manager = null;
var dataUtil = require("../../data-util/sp-master-plan/standard-hour-data-util");
...
var BuyerManager = require("../../../src/managers/master/sp-buyer-manager");
var buyerManager = null;
var BuyerdataUtil = require("../../data-util/master/sp-buyer-data-util");
...
it(`#01. should success when create new data`, function (done) {
    dataUtil.getNewData()
    .then((data) => {
            // data.spBuyerId = createdIdBuyer;
            // data.spBuyer=createdDataBuyer;
...
it(`#02. should success when get sh by filter from created data`, function (done) {
    var buyerCode=createdData.spBuyerCode;
...
it(`#05. should success when destroy buyer data with id`, function(done) {
    buyerManager.destroy(createdData.spBuyerId)
        .then((result) => {
            result.should.be.Boolean();
            result.should.equal(true);
            done();
...
it(`#07. should null when get destroyed buyer data`, function(done) {
    buyerManager.getSingleByIdOrDefault(createdData.spBuyerId)
        .then((data) => {
            should.equal(data, null);
            done();
        })
