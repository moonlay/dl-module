...
it("#13. should success when create new 10 data Production Order with having daily operation", function (done) {
    var dataReport = [];
    for (var a = 0; a < 5; a++) {
        var data = dataUtil.getNewData({ buyer: dataBuyer1, process: dataProcessType1, account: dataAccount1 });
        dataReport.push(data);
            ...
            Promise.all(createData)
                .then(created => {
                    ...
                    for (var id of created) {
                        getData.push(manager.getSingleById(id))
                    }
                    Promise.all(getData)
                        .then((results) => {
                            var createDailyOp = [];
                            for (var result of results) {
                                createDailyOp.push(dailyOperationDataUtil.getNewTestDataExistingProductionOrder(result))
                            }
                            Promise.all(createDailyOp)
                                .then((data) => {
                                    var daily = data;
                                    done();
                                })
