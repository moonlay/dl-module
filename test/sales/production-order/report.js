...
before('#00. connect db', function (done) {
    helper.getDb()
        .then(db => {
            ...
            dailyOperationManager = new DailyOperationManager(db, { username: 'dev' });
            done();
        })
        .catch(e => {
            done(e);
...
var dataAccount1;
var dataAccount2;
var dataDailyOperation1;
var dataDailyOperation2;
...
it("#01. should success when create new support data (buyer, account, process type)", function (done) {
    ...
    var dataDailyOperation1 = dailyOperationDataUtil.getNewData("input");
    Promise.all([processType1, buyer1, account1, dataDailyOperation1])
        .then(data1 => {
            ...
            var dataDailyOperation2 = dailyOperationDataUtil.getNewData("input");
            Promise.all([processType2, buyer2, account2, dataDailyOperation2])
                .then(data2 => {
                    Promise.all([processTypeManager.create(data1[0]), processTypeManager.create(data2[0]), buyerManager.create(data1[1]), buyerManager.create(data2[1]), accountManager.create(data1[2]), accountManager.create(data2[2]), dailyOperationManager.create(data1[3]), dailyOperationManager.create(data2[3])])
                        .then(id => {
                            Promise.all([processTypeManager.getSingleById(id[0]), processTypeManager.getSingleById(id[1]), buyerManager.getSingleById(id[2]), buyerManager.getSingleById(id[3]), accountManager.getSingleById(id[4]), accountManager.getSingleById(id[5]), dailyOperationManager.getSingleById(id[6]), dailyOperationManager.getSingleById(id[7])])
                                .then(results => {
                                    ...
                                    validate.production.finishingPrinting.dailyOperation(results[6]);
                                    dataDailyOperation1 = results[6];
                                    validate.production.finishingPrinting.dailyOperation(results[7]);
                                    dataDailyOperation2 = results[7];
                                    done();
                                })
                                .catch(e => {
                                    done(e);
...
it("#04. should success when create new 10 data Production Order with 2 detail color in each data production order", function (done) {
    var dataReport = [];
    for (var a = 0; a < 5; a++) {
        var data = dataUtil.getNewData({ buyer: dataBuyer1, process: dataProcessType1, account: dataAccount1, dailyOperation: dataDailyOperation1 });
        dataReport.push(data);
    }
    for (var a = 0; a < 5; a++) {
        var data = dataUtil.getNewData({ buyer: dataBuyer2, process: dataProcessType2, account: dataAccount2, dailyOperation: dataDailyOperation2 });
        dataReport.push(data);
    }
