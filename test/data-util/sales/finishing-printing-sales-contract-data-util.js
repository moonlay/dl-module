...
class FinishingPrintingSalesContractDataUtil {
    getNewData() {
        ...
        return material.getTestData()
            .then(material => {
                _material = material;

                return Promise.all([uom.getTestData(), buyer.getTestData(), quality.getTestData(), processType.getTestData(), Promise.resolve(null), comodity.getTestData(), yarnMaterial.getTestData(), materialConstruction.getTestData(), bankAccount.getTestData(), termOfPayment.getTestData(), motive.getTestData()])
                    .then((results) => {
                        var _uom = results[0];
