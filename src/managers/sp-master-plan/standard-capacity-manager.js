...
module.exports = class StandardCapacityManager extends BaseManager {
    ...
    _validate(standardCapacity) {
        ...
        var getStandarCapacity = this.collection.where({ 
            ...
            "_deleted": false,
            "spBuyerId": valid.spBuyerId && ObjectId.isValid(valid.spBuyerId) ? new ObjectId(valid.spBuyerId) : '',
        ...
        return Promise.all([getStandarCapacity, getBuyer, getComodity])
            .then(results => {
                ...
                if(!valid.spBuyerId || valid.spBuyerId==='')
                    errors["buyer"] = i18n.__("StandardCapacity.spBuyerId.isRequired:%s is required", i18n.__("StandardCapacity.spBuyerId._:Buyer"));
                ...
                if(!valid.shFinishing || valid.shFinishing <= 0)
                    errors["shFinishing"] = i18n.__("StandardCapacity.shFinishing.mustBeGreater:%s must be greater than 0", i18n.__("StandardCapacity.SMVFinishing._:SMVFinishing"));
                ...
                if(_buyer){
                    valid.spBuyerId = new ObjectId(_buyer._id);
    ...
    getStandardCapacityByBuyerComodity(buyerCode, comodityCode){
        return new Promise((resolve, reject) => {
            this.collection.aggregate(
                [
                    { $match: { "masterplanComodityCode":comodityCode , spBuyerCode:buyerCode} },
                    { $sort: { date:-1, _updatedDate:-1 } },
                    {
                    $group:
                        {
                        _id:{ "masterplanComodityCode":comodityCode , spBuyerCode:buyerCode},
                        firstSHSewing: { $first: "$shSewing" },
                        shId: { $first: "$_id" }
                        //shId: "$_id"
                        }
