...
var SewingBlockingPlan = DLModels.spMasterPlan.SewingBlockingPlan;
var SewingBlockingPlanDetail = DLModels.spMasterPlan.SewingBlockingPlanDetail;
var SewingBlockingPlanDetailItem = DLModels.spMasterPlan.SewingBlockingPlanDetailItem;
...
module.exports = class SewingBlockingPlanManager extends BaseManager {
    constructor(db, user) {
        super(db, user);
        this.collection = this.db.use(map.spMasterPlan.collection.SewingBlockingPlan);
        this.weeklyPlanManager = new WeeklyPlanManager(db, user);
        this.spBuyerManager = new SpBuyerManager(db, user);
    ...
    _beforeInsert(spBlockingPlan) {
        spBlockingPlan.code = !spBlockingPlan.code ? generateCode() : spBlockingPlan.code;
        spBlockingPlan._active = true;
        spBlockingPlan._createdDate = new Date();
        return Promise.resolve(spBlockingPlan);
    }

    _validate(spBlockingPlan) {
        var errors = {};
        var valid = spBlockingPlan;
        ...
        var getBuyer = valid.spBuyerId && ObjectId.isValid(valid.spBuyerId) ? this.spBuyerManager.getSingleByIdOrDefault(new ObjectId(valid.spBuyerId)) : Promise.resolve(null);
        ...
        return Promise.all([getSewingBlockingPlan, getBuyer, getBookingOrder, getUnits, getWeeklyPlan, getComodity])
            .then(results => {
                ...
                if (_buyer) {
                    valid.spBuyerId = _buyer._id;
                    valid.spBuyerName = _buyer.name;
                    valid.spBuyerCode = _buyer.code;
                }
    ...
    _createIndexes() {
        var dateIndex = {
            name: `ix_${map.spMasterPlan.collection.SewingBlockingPlan}__updatedDate`,
            key: {
                _updatedDate: -1
            }
        };
        ...
        var codeIndex = {
            name: `ix_${map.spMasterPlan.collection.SewingBlockingPlan}_code`,
            key: {
                "code": 1
            }
        };
    ...
    getReport(query) {
        return new Promise((resolve, reject) => {
            ...
            var weeklyPlans = map.spMasterPlan.collection.WeeklyPlan;
            var bookingOrders = map.spMasterPlan.collection.BookingOrder;
            ...
            this.collection
                .aggregate([
                    ...
                    {
                        "$project": {
                            "buyer": { $concat: ["$spBuyerName", "-", "$details.masterPlanComodity.name"] },
    ...
    getAcceptedOrderMonitoringXls(dataReport, query) {
        return new Promise((resolve, reject) => {
            ...
            var Query = { "$and": [ deletedQuery, yearQuery, unitQuery] };
            var getUnit=   this.db.use(map.spMasterPlan.collection.WeeklyPlan)
            .aggregate([
                { "$match": Query },
                { "$project": {
                    'unit' :'$unit.code', 
