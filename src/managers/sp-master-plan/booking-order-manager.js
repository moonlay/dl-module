...
var SpBuyerManager = require('../master/sp-buyer-manager');
var SpSectionManager = require('../sp-master-plan/sp-section-manager');
...
module.exports = class BookingOrderManager extends BaseManager {
    constructor(db, user) {
        ...
        this.spBlockingPlanCollection = this.db.use(map.garmentMasterPlan.collection.SewingBlockingPlan);
        this.comodityManager = new ComodityManager(db, user);
        //this.masterPlanManager = new MasterPlanManager(db, user);
        this.garmentBuyerManager = new SpBuyerManager(db, user);
        this.garmentSectionManager = new SpSectionManager(db, user);
        ...
        return Promise.all([getBooking,getBuyer,getSection])
            .then(results => {
                ...
                if (!valid.deliveryDate || valid.deliveryDate === "") {
                    ...
                    valid.bookingDate.setUTCCapacities(valid.bookingDate.getUTCCapacities() >= 17 ? 17 : -7, 0, 0, 0);
                    valid.deliveryDate.setUTCCapacities(valid.deliveryDate.getUTCCapacities() >= 17 ? 17 : -7, 0, 0, 0);

                    // setUTCCapacities 17 atau -7 untuk mengubah ke 00:00:00 WIB

                    var next45Days = new Date();
                    next45Days.setUTCCapacities(next45Days.getUTCCapacities() >= 17 ? 17 : -7, 0, 0, 0);
                    ...
                    if (valid.items.length > 0) {
                        ...
                        for (var item of valid.items) {
                            ...
                            if (!item.deliveryDate || item.deliveryDate === "") {
                                ...
                                check_bookingDate.setUTCCapacities(check_bookingDate.getUTCCapacities() >= 17 ? 17 : -7, 0, 0, 0);
                                var check_deliveryDate = new Date(valid.deliveryDate);
                                check_deliveryDate.setUTCCapacities(check_deliveryDate.getUTCCapacities() >= 17 ? 17 : -7, 0, 0, 0);
                                var check_item_deliveryDate = new Date(item.deliveryDate);
                                check_item_deliveryDate.setUTCCapacities(check_item_deliveryDate.getUTCCapacities() >= 17 ? 17 : -7, 0, 0, 0);
    ...
    expiredBooking(booking){
        return this.getSingleById(booking._id)
            .then( (booking) => {
                ...
                return this.spBlockingPlanCollection.singleOrDefault(query)
                    .then((spBlockingPlan) => {
                        var total=0;
                        ...
                        if(spBlockingPlan){
                            if(booking.items.length==0){
                                spBlockingPlan.status="Booking Expired";
                            }
                            else{
                                spBlockingPlan.status="Booking Ada Perubahan";
                                for(var detail of spBlockingPlan.details){
                            ...
                            return this.spBlockingPlanCollection.update(spBlockingPlan)
                                .then((id) =>{
    ...
    _beforeInsert(data) {
        ...
        var dataSpSectionCode = data.garmentSectionCode ? data.garmentSectionCode : "";
        var dataSpBuyerCode = data.garmentBuyerCode ? data.garmentBuyerCode : "";
        var type = dataSpSectionCode + "-" + dataSpBuyerCode;
        ...
        return new Promise((resolve, reject) => {
            ...
            this.getSingleById(id)
                .then((booking) => {
                    ...
                    this.spBlockingPlanCollection.singleOrDefault(query)
                        .then((spBlockingPlan) => {
                            if(spBlockingPlan){
                                if(booking.isCanceled){
                                    spBlockingPlan.status="Booking Dibatalkan";
                                }
                                else{
                                    spBlockingPlan.status="Booking Ada Perubahan";
                                    for(var detail of spBlockingPlan.details){
                                ...
                                this.spBlockingPlanCollection.update(spBlockingPlan)
                                    .then((id) =>
                                        resolve(bookingId));
                            }
                            else{
    ...
    delete(data) {
        data._deleted = true;
        return this.spBlockingPlanCollection.singleOrDefault({ "bookingOrderNo": data.code,"_deleted":false })
                   .then(spBlockingPlan =>{
                       if(spBlockingPlan){
                        spBlockingPlan.status="Booking Dihapus";
                        return this.spBlockingPlanCollection.update(spBlockingPlan)
    ...
    getBookingOrderReport(query,offset) {
        return new Promise((resolve, reject) => {
            ...
            dateFrom.setUTCCapacities(dateFrom.getUTCCapacities() >= 17 ? 17 : -7, 0, 0, 0);
            dateTo.setUTCCapacities(dateTo.getUTCCapacities() >= 17 ? 17 : -7, 0, 0, 0);
    ...
    getBookingOrderReportXls(dataReport, query,offset) {
        ...
        return new Promise((resolve, reject) => {
            ...
            for (var data of dataReport.data) {
                ...
                var b = today;
                a = new Date(a);
                a.setCapacities(0,0,0,0);
                b.setCapacities(0,0,0,0);
